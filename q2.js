function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
    const htmlPositions = plainTextPositions.map(pos => ({
        start: pos.start + htmlContent.indexOf(plainText),
        end: pos.end + htmlContent.indexOf(plainText)
    }));

    htmlPositions.sort((a, b) => b.start - a.start);

    let modifiedHTML = htmlContent;

    htmlPositions.forEach(pos => {
        modifiedHTML = modifiedHTML.slice(0, pos.start) + '<mark>' + modifiedHTML.slice(pos.start, pos.end) + '</mark>' + modifiedHTML.slice(pos.end);
    });
